...
export const parameters = z.object({
    ...
    requestType: z
        .preprocess((v) => (typeof v === 'string' ? v.toUpperCase() : v), z.enum(['REQUEST_TO_ADD_MEMBERS', 'REQUEST_TO_JOIN', 'UPGRADE_ROLE']))
...
export async function handler(args, extra) {
    try {
        ...
        if (args.entityList !== undefined)
            bodyPayload.entityList = args.entityList;
        if (args.role !== undefined)
            bodyPayload.role = args.role;
        if (args.reason !== undefined)
            bodyPayload.reason = args.reason;
        if (args.requestType !== undefined)
            bodyPayload.requestType = args.requestType;
        const options = {
