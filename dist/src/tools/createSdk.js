...
export const parameters = z.object({
    ...
    sdkVersion: z
        .string()
        .describe("An optional override for the SDK version. If this isn't provided, the next version is resolved from the latest auto-pull request.")
        .optional(),
    authors: z
        .array(z
        .object({
    ...
    retry: z
        ...
        httpMethodsToRetry: z
            .array(z.preprocess((v) => (typeof v === 'string' ? v.toUpperCase() : v), z.enum(['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS', 'CONNECT', 'TRACE'])))
            .describe('A list of the HTTP methods that the SDK attempts its retries on.')
            .optional(),
    })
        .describe("Retry behavior baked into the generated SDK's HTTP client. A power-user option; sensible defaults apply for any field left unset.")
        .optional(),
    typescriptOptions: z
        .object({
        npmOrg: z
...
export async function handler(args, extra) {
    try {
        ...
        if (args.sdkVersion !== undefined)
            bodyPayload.sdkVersion = args.sdkVersion;
        if (args.authors !== undefined)
            bodyPayload.authors = args.authors;
        if (args.retry !== undefined)
            bodyPayload.retry = args.retry;
        if (args.typescriptOptions !== undefined)
            bodyPayload.typescriptOptions = args.typescriptOptions;
        if (args.pythonOptions !== undefined)
            bodyPayload.pythonOptions = args.pythonOptions;
        if (args.goOptions !== undefined)
            bodyPayload.goOptions = args.goOptions;
        if (args.javaOptions !== undefined)
            bodyPayload.javaOptions = args.javaOptions;
        if (args.csharpOptions !== undefined)
            bodyPayload.csharpOptions = args.csharpOptions;
        if (args.rubyOptions !== undefined)
            bodyPayload.rubyOptions = args.rubyOptions;
        if (args.phpOptions !== undefined)
            bodyPayload.phpOptions = args.phpOptions;
        if (args.kotlinOptions !== undefined)
            bodyPayload.kotlinOptions = args.kotlinOptions;
        if (args.rustOptions !== undefined)
            bodyPayload.rustOptions = args.rustOptions;
        if (args.cliOptions !== undefined)
            bodyPayload.cliOptions = args.cliOptions;
        const options = {
