...
export const parameters = z.object({
    ...
    include: z
        .string()
        .describe('The additional fields to be included as a part of the request:\n- `meta.total` — Include the total records found in the `meta` response object.\n')
        .optional(),
    since: z
        .string()
    ...
    secretTypes: z
        ...
        .describe('A list of secrets types to query. For a list of valid IDs, use the GET `/secret-types` endpoint.')
        .optional(),
    statuses: z
        .array(z
        .preprocess((v) => (typeof v === 'string' ? v.toUpperCase() : v), z.enum(['FALSE_POSITIVE', 'ACCEPTED_RISK', 'REVOKED', 'ACTIVE']))
        .describe("The secret's resolution status:\n- `ACTIVE` — The secret is active.\n- `FALSE_POSITIVE` — The discovered secret is not an actual secret.\n- `REVOKED` — The secret is valid, but the user rotated their key to resolve the issue.\n- `ACCEPTED_RISK` — The Secret Scanner found the secret, but user accepts the risk of publishing it.\n"))
    ...
    resources: z
        ...
        .describe('A list of resources to query. If you use this query, you cannot also pass the `workspaceIds` query.')
        .optional(),
    workspaceIds: z
        .array(z.string().describe('The workspace ID.'))
        .describe('A list of workspaces IDs to query. If you use this query, you cannot also pass the `resources` query.')
        .optional(),
    workspaceVisibilities: z
        .array(z.enum(['team', 'public']).describe('The type of visibility setting.'))
        .describe('A list of workspace [visibility settings](https://learning.postman.com/docs/collaborating-in-postman/using-workspaces/managing-workspaces/#changing-workspace-visibility) to query. This currently supports the `team` and `public` settings.')
        .optional(),
});
...
export async function handler(args, extra) {
    try {
        ...
        if (args.resolved !== undefined)
            bodyPayload.resolved = args.resolved;
        if (args.secretTypes !== undefined)
            bodyPayload.secretTypes = args.secretTypes;
        if (args.statuses !== undefined)
            bodyPayload.statuses = args.statuses;
        if (args.resources !== undefined)
            bodyPayload.resources = args.resources;
        if (args.workspaceIds !== undefined)
            bodyPayload.workspaceIds = args.workspaceIds;
