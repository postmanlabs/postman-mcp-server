...
export const parameters = z.object({
    discoverySource: z
        ...
        .describe('Filter results by the given discovery source type:\n- `api_gateway_app` — API Gateway integrations.\n- `insights_project` — Postman Insights.\n- `infra_watcher` — Cluster Watcher.\n- `public_api` — Services manually added using the POST `/api-catalog/discovery-services` endpoint.\n')
        .optional(),
    status: z
        .string()
        .describe('A comma-separated list of statuses to filter the results by. Accepts the `discovered`, `integrated`, and `archived` values.')
        .optional(),
    search: z
        .string()
        .describe("Filter results by the given value that match a service's name. Matching is not case-sensitive.")
        .optional(),
    limit: z
        .number()
        .int()
