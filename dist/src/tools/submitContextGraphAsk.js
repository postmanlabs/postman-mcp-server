...
export const parameters = z.object({
    ...
    includeAnswer: z
        .boolean()
        .describe("Whether to include the prose answer in the result. If `false`, the result's `answer` property is `null` and the result's other properties are unaffected.")
        .default(true),
    maxSteps: z
        .number()
        .int()
...
export async function handler(args, extra) {
    try {
        ...
        if (args.query !== undefined)
            bodyPayload.query = args.query;
        if (args.includeAnswer !== undefined)
            bodyPayload.includeAnswer = args.includeAnswer;
        if (args.maxSteps !== undefined)
            bodyPayload.maxSteps = args.maxSteps;
        const options = {
