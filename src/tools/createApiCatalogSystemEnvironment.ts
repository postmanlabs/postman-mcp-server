...
export const parameters = z.object({
  ...
  label: z
    ...
    .max(50)
    .describe(
      'A lowercase, terminal-friendly identifier for the system environment. Accepts only alphanumeric characters, hyphens, and underscores.'
    )
    .optional(),
  description: z
    .string()
    .max(500)
    .describe(
      'A description of the system environment. To remove a description, pass this value as an empty string.'
    )
    .optional(),
...
export async function handler(
  ...
  try {
    ...
    if (args.label !== undefined) bodyPayload.label = args.label;
    if (args.description !== undefined) bodyPayload.description = args.description;
    if (args.color !== undefined) bodyPayload.color = args.color;
    if (args.isProduction !== undefined) bodyPayload.isProduction = args.isProduction;
    const options: any = {
