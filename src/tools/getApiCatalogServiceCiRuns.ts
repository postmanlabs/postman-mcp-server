...
export const parameters = z.object({
  ...
  sort: z
    ...
    .describe(
      'Sort the results in field:direction order format. Accepts the `timestamp` and `duration` fields. Supports the `asc` and `desc` directions.'
    )
    .optional(),
  limit: z
    .number()
...
export async function handler(
  ...
  try {
    ...
    if (args.collectionId !== undefined) query.set('collectionId', String(args.collectionId));
    if (args.environmentId !== undefined) query.set('environmentId', String(args.environmentId));
    if (args.status !== undefined) query.set('status', String(args.status));
    if (args.branch !== undefined) query.set('branch', String(args.branch));
    if (args.workflowName !== undefined) query.set('workflowName', String(args.workflowName));
    if (args.actor !== undefined) query.set('actor', String(args.actor));
    if (args.repoName !== undefined) query.set('repoName', String(args.repoName));
    if (args.repoOwner !== undefined) query.set('repoOwner', String(args.repoOwner));
