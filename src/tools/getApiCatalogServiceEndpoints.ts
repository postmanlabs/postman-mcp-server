...
export const parameters = z.object({
  ...
  sort: z
    ...
    .describe(
      'Sort the results in field:direction order format. Accepts the `count`, `endpoint`, `p95LatencyMs`, and `errorRate` fields. Supports the `asc` and `desc` directions.'
    )
    .optional(),
  limit: z
    .number()
...
export async function handler(
  ...
  try {
    ...
    if (args.httpMethods !== undefined) query.set('httpMethods', String(args.httpMethods));
    if (args.hosts !== undefined) query.set('hosts', String(args.hosts));
    if (args.responseCodes !== undefined) query.set('responseCodes', String(args.responseCodes));
