...
export const parameters = z.object({
  ...
  severity: z
    ...
    .describe(
      'Filter results to runs with issues at or above the given severity threshold. Higher severities are always included.'
    )
    .optional(),
  status: z
    .enum(['passed', 'failed'])
  ...
  sort: z
    ...
    .describe(
      'Sort the results in field:direction order format. Accepts the `timestamp` and `errorCount` fields. Supports the `asc` and `desc` directions.'
    )
    .optional(),
  limit: z
    .number()
