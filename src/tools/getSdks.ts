...
export const parameters = z.object({
  ...
  sdkIds: z
    ...
    .max(100)
    .describe(
      'A comma-separated list of SDK IDs to return in the response. If you pass this query parameter with other filters, the other filters are ignored.'
    )
    .optional(),
  buildStatus: z
...
export async function handler(
  ...
  try {
    ...
    if (args.sdkIds !== undefined) query.set('sdkIds', String(args.sdkIds));
    if (args.buildStatus !== undefined) query.set('buildStatus', String(args.buildStatus));
    if (args.language !== undefined) query.set('language', String(args.language));
    if (args.sourceId !== undefined) query.set('sourceId', String(args.sourceId));
