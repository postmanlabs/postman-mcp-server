...
export const parameters = z.object({
  ...
  cursor: z
    .string()
    .describe(
      'The cursor to get the next set of results in a paginated response. Get this value from the `meta.nextCursor` field in the previous response.\n'
    )
    .optional(),
  limit: z
    .number()
...
export async function handler(
  ...
  try {
    ...
    if (args.elementType !== undefined) query.set('elementType', String(args.elementType));
    if (args.elementId !== undefined) query.set('elementId', String(args.elementId));
