...
export const description = `Search for Postman entities (requests, collections, workspaces, specs, and flows).
...
| \`isGitConnected\` | \`$eq\`, \`$ne\` | Boolean. Workspaces, collections, requests, specs, flows. |
...
/** Boolean operator: { "$eq": boolean } or { "$ne": boolean } */
...
/** String operator: $eq/$ne (single string) or $in/$nin (array of strings) */
...
/** Visibility operator: $eq/$ne restricted to "internal" | "public" | "partner" */
...
/**
 * A single filter condition — exactly one field key mapped to its operator.
 * The API requires each condition object to have exactly one property.
 */
...
/** Zod schema for the full filters object: { "$and": [{ "<field>": { "<op>": <val> } }] } */
const filtersSchema = z
  .object({
    $and: z
      ...
      .min(1)
      .describe(
        'Array of filter conditions. Each condition must contain exactly one field key mapped to an operator object.'
      ),
  })
  .describe(
    'Structured filter expression. Top-level key must be "$and" with an array of condition objects. ' +
...
export const parameters = z.object({
  entityType: z
    .enum(['requests', 'collections', 'workspaces', 'specs', 'flows'])
    .describe(
      'The type of Postman entity to search for: `requests` (individual API requests), `collections` (API collections), `workspaces` (Postman workspaces), `specs` (API specifications), or `flows` (Postman Flows).'
    )
    .default('requests'),
  q: z
    .string()
  ...
  ownership: z
    ...
    .describe(
      'The ownership scope. Use `organization` to search all resources in your organization (default), `external` to search the public Postman network, or `all` to search across all scopes.'
    )
  ...
  cursor: z
    .string()
    .describe(
      'The cursor to get the next set of results in the paginated response. Pass the `nextCursor` value from the previous response.'
    )
    .optional(),
  limit: z
    .number()
...
export const annotations = {
  title: 'Search for Postman entities (requests, collections, workspaces, specs, flows).',
  readOnlyHint: true,
...
export async function handler(
  ...
  try {
    ...
    if (args.cursor !== undefined) query.set('nextCursor', String(args.cursor));
