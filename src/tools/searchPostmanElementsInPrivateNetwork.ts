...
export const parameters = z.object({
  entityType: z
    ...
    .describe(
      'The type of Postman element to search for. Use `requests` to search for individual API requests, or `collections` to search for collections (unique collections extracted from request results; pagination applies to underlying requests).'
    )
    .default('requests'),
  q: z
    .string()
    ...
    .max(512)
    .describe(
      'The search query for API elements in the Private API Network (e.g. "invoices API", "notification service", "payment APIs").'
    ),
  nextCursor: z
    .string()
    .describe(
...
export const annotations = {
  title:
    "Search for API requests and collections in your organization's Private API Network—a curated repository of trusted, internal APIs shared by your team.",
  readOnlyHint: true,
  destructiveHint: false,
  idempotentHint: true,
};

/** Applied server-side to scope results to the Private API Network. Not exposed to the user. */
            ...
            text: JSON.stringify(
              {
                data: collections,
                meta: {
                  nextCursor: (result as any)?.meta?.nextCursor,
