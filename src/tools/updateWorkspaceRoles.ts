            ...
            z
              .object({
                id: z
                  .string()
                  .describe(
                    "The user, user group, or partner's ID. To use SCIM IDs for users or user groups, include the `identifierType=scim` header in the request."
                  ),
                role: z
                  .string()
                  .describe(
...
export async function handler(
  ...
  try {
    ...
    if (args.roles !== undefined) bodyPayload.roles = args.roles;
    const options: any = {
